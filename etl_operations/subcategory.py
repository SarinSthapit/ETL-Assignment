...
def handle_data_update(cursor, staging_table, temporary_table, key_column):
    query = f"""
            ...
            WHEN NOT MATCHED THEN 
                INSERT (tmp.id, tmp.category_key, tmp.subcategory_desc)
                VALUES (stg.id, stg.category_id, stg.subcategory_desc)
            WHEN MATCHED THEN UPDATE SET 
                tmp.id = stg.id, 
                tmp.category_key = stg.category_id,
                tmp.subcategory_desc = stg.subcategory_desc
        """
...
def reclassify_and_add_rows(cursor, staging_table, temporary_table, key_column):
    query = f"""
            INSERT INTO {temporary_table} (id, category_key, category_desc)
            SELECT stg.{key_column}, stg.category_id, stg.subcategory_desc
...
def handle_closing_dimension(cursor, temporary_table, target_table, key_column):
    ...
    query = f"""
            MERGE INTO {target_table} AS tgt
                ...
                WHEN NOT MATCHED THEN 
                    INSERT (subcategory_key, id, category_key, subcategory_desc, active_flag, created_at, updated_at)
                    VALUES (
